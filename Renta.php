    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-8"><h2>Renta <b>FAADPRO</b></h2></div>
                    <div class="col-sm-4">
                        <a href='./insertar/insertarre.html' class="btn btn-info add-new"><i class="fa fa-plus"></i> Añadir Renta</a>
                    </div>
                </div>
            </div>
            <?php
...
$sql = "SELECT id, mes, duracion, termino FROM renta";
...
if ($result->num_rows > 0) {
  ...
  echo "<tr>";
  echo "<th>ID</th><th>mes</th><th>duracion</th><th>termino</th><th>acciones</th>";
  echo "</tr>";
  while($row = $result->fetch_assoc()) {
            echo "<tr>";
            echo "<td>".$row["id"]."</td><td>".$row["mes"]."</td><td>".$row["duracion"]."</td><td>".$row["termino"]."</td>";
            echo "<td>
            <a class='edit' title='Editar' data-toggle='tooltip' href='./modificar/formmodrenta.php?id=".$row["id"]."&mes=".$row["mes"]."&duracion=".$row["duracion"]."&termino=".$row["termino"]."' ><i class='material-icons'>&#xE254;</i></a>
            <a class='delete' title='Eliminar' data-toggle='tooltip' href='./eliminar/eliminarre.php?id=".$row["id"]."' ><i class='material-icons'>&#xE872;</i></a>
        </td>";
            echo "</tr>";
  }
} else {
