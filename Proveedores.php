    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-8"><h2>proveedores <b>FAADPRO</b></h2></div>
                    <div class="col-sm-4">
                        <a href='./insertar/insertarpro.html' class="btn btn-info add-new"><i class="fa fa-plus"></i> Añadir proveedores</a>
                    </div>
                </div>
            </div>
            <?php
...
$sql = "SELECT id, nombre, apellido, telefono, domicilio FROM proveedores";
...
if ($result->num_rows > 0) {
  ...
  echo "<tr>";
  echo "<th>ID</th><th>nombre</th><th>apellido</th><th>telefono</th><th>domicilio</th><th>acciones</th>";
  echo "</tr>";
  while($row = $result->fetch_assoc()) {
            echo "<tr>";
            echo "<td>".$row["id"]."</td><td>".$row["nombre"]."</td><td>".$row["apellido"]."</td><td>".$row["telefono"]."</td><td>".$row["domicilio"]."</td>";
            echo "<td>
            <a class='edit' title='Editar' data-toggle='tooltip' href='./modificar/formmodproveedores.php?id=".$row["id"]."&nombre=".$row["nombre"]."&apellido=".$row["apellido"]."&telefono=".$row["telefono"]."&domicilio=".$row["domicilio"]."' ><i class='material-icons'>&#xE254;</i></a>
            <a class='delete' title='Eliminar' data-toggle='tooltip' href='./eliminar/eliminarpro.php?id=".$row["id"]."' ><i class='material-icons'>&#xE872;</i></a>
        </td>";
            echo "</tr>";
  }
} else {
