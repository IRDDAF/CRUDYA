    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-8"><h2>Promociones <b>FAADPRO</b></h2></div>
                    <div class="col-sm-4">
                        <a href='./insertar/insertarp.html' class="btn btn-info add-new"><i class="fa fa-plus"></i> Añadir promociones</a>
                    </div>
                </div>
            </div>
            <?php
...
$sql = "SELECT id, nombre, descripcion, descuento, vigencia FROM promociones";
...
if ($result->num_rows > 0) {
  ...
  echo "<tr>";
  echo "<th>ID</th><th>nombre</th><th>descripción</th><th>descuento</th><th>vigencia</th><th>acciones</th>";
  echo "</tr>";
  while($row = $result->fetch_assoc()) {
            echo "<tr>";
            echo "<td>".$row["id"]."</td><td>".$row["nombre"]."</td><td>".$row["descripcion"]."</td><td>".$row["descuento"]."%</td><td>".$row["vigencia"]."</td>";
            echo "<td>
            <a class='edit' title='Editar' data-toggle='tooltip' href='./modificar/formmodpromociones.php?id=".$row["id"]."&nombre=".$row["nombre"]."&descripcion=".$row["descripcion"]."&descuento=".$row["descuento"]."&vigencia=".$row["vigencia"]."' ><i class='material-icons'>&#xE254;</i></a>
            <a class='delete' title='Eliminar' data-toggle='tooltip' href='./eliminar/eliminarp.php?id=".$row["id"]."' ><i class='material-icons'>&#xE872;</i></a>
        </td>";
            echo "</tr>";
  }
} else {
