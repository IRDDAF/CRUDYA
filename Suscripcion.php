    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-8"><h2>Suscripcion <b>FAADPRO</b></h2></div>
                    <div class="col-sm-4">
                        <a href='./insertar/insertarsu.html' class="btn btn-info add-new"><i class="fa fa-plus"></i> Añadir Suscripcion</a>
                    </div>
                </div>
            </div>
            <?php
...
$sql = "SELECT id, nombre, apellidos, nocuenta FROM suscripcion";
...
if ($result->num_rows > 0) {
  ...
  echo "<tr>";
  echo "<th>ID</th><th>nombre</th><th>apellidos</th><th>nocuenta</th><th>acciones</th>";
  echo "</tr>";
  while($row = $result->fetch_assoc()) {
            echo "<tr>";
            echo "<td>".$row["id"]."</td><td>".$row["nombre"]."</td><td>".$row["apellidos"]."</td><td>".$row["nocuenta"]."</td>";
            echo "<td>
            <a class='edit' title='Editar' data-toggle='tooltip' href='./modificar/formmodsuscripcion.php?id=".$row["id"]."&nombre=".$row["nombre"]."&apellidos=".$row["apellidos"]."&nocuenta=".$row["nocuenta"]."' ><i class='material-icons'>&#xE254;</i></a>
            <a class='delete' title='Eliminar' data-toggle='tooltip' href='./eliminar/eliminarsu.php?id=".$row["id"]."' ><i class='material-icons'>&#xE872;</i></a>
        </td>";
            echo "</tr>";
  }
} else {
