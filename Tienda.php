    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-8"><h2>Tienda <b>FAADPRO</b></h2></div>
                    <div class="col-sm-4">
                        <a href='./insertar/insertart.html' class="btn btn-info add-new"><i class="fa fa-plus"></i> Añadir Tienda</a>
                    </div>
                </div>
            </div>
            <?php
...
$sql = "SELECT id, nombre, ubicacion, horario, telefono FROM tienda";
...
if ($result->num_rows > 0) {
  ...
  echo "<tr>";
  echo "<th>ID</th><th>nombre</th><th>ubicacion</th><th>horario</th><th>telefono</th><th>acciones</th>";
  echo "</tr>";
  while($row = $result->fetch_assoc()) {
            echo "<tr>";
            echo "<td>".$row["id"]."</td><td>".$row["nombre"]."</td><td>".$row["ubicacion"]."</td><td>".$row["horario"]."</td><td>".$row["telefono"]."</td>";
            echo "<td>
            <a class='edit' title='Editar' data-toggle='tooltip' href='./modificar/formmodtienda.php?id=".$row["id"]."&nombre=".$row["nombre"]."&ubicacion=".$row["ubicacion"]."&horario=".$row["horario"]."&telefono=".$row["telefono"]."' ><i class='material-icons'>&#xE254;</i></a>
            <a class='delete' title='Eliminar' data-toggle='tooltip' href='./eliminar/eliminart.php?id=".$row["id"]."' ><i class='material-icons'>&#xE872;</i></a>
        </td>";
            echo "</tr>";
  }
} else {
