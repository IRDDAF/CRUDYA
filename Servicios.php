    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-8"><h2>Servicios <b>FAADPRO</b></h2></div>
                    <div class="col-sm-4">
                        <a href='./insertar/insertars.html' class="btn btn-info add-new"><i class="fa fa-plus"></i> Añadir Servicio</a>
                    </div>
                </div>
            </div>
            <?php
...
$sql = "SELECT id, tipo, contacto, costo FROM servicios";
...
if ($result->num_rows > 0) {
  ...
  echo "<tr>";
  echo "<th>ID</th><th>tipo</th><th>contacto</th><th>costo</th><th>acciones</th>";
  echo "</tr>";
  while($row = $result->fetch_assoc()) {
            echo "<tr>";
            echo "<td>".$row["id"]."</td><td>".$row["tipo"]."</td><td>".$row["contacto"]."</td><td>$".$row["costo"]."</td>";
            echo "<td>
            <a class='edit' title='Editar' data-toggle='tooltip' href='./modificar/formmodservicios.php?id=".$row["id"]."&tipo=".$row["tipo"]."&contacto=".$row["contacto"]."&costo=".$row["costo"]."' ><i class='material-icons'>&#xE254;</i></a>
            <a class='delete' title='Eliminar' data-toggle='tooltip' href='./eliminar/eliminars.php?id=".$row["id"]."' ><i class='material-icons'>&#xE872;</i></a>
        </td>";
            echo "</tr>";
  }
} else {
