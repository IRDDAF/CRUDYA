    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-8"><h2>Refacciones <b>FAADPRO</b></h2></div>
                    <div class="col-sm-4">
                        <a href='./insertar/insertarr.html' class="btn btn-info add-new"><i class="fa fa-plus"></i> Añadir Refacciones</a>
                    </div>
                </div>
            </div>
            <?php
...
$sql = "SELECT id, nombre, modelo, costo, garantia FROM refacciones";
...
if ($result->num_rows > 0) {
  ...
  echo "<tr>";
  echo "<th>ID</th><th>nombre</th><th>modelo</th><th>costo</th><th>garantia</th><th>acciones</th>";
  echo "</tr>";
  while($row = $result->fetch_assoc()) {
            echo "<tr>";
            echo "<td>".$row["id"]."</td><td>".$row["nombre"]."</td><td>".$row["modelo"]."</td><td>$".$row["costo"]."</td><td>".$row["garantia"]."</td>";
            echo "<td>
            <a class='edit' title='Editar' data-toggle='tooltip' href='./modificar/formmodrefacciones.php?id=".$row["id"]."&nombre=".$row["nombre"]."&modelo=".$row["modelo"]."&costo=".$row["costo"]."&garantia=".$row["garantia"]."' ><i class='material-icons'>&#xE254;</i></a>
            <a class='delete' title='Eliminar' data-toggle='tooltip' href='./eliminar/eliminarr.php?id=".$row["id"]."' ><i class='material-icons'>&#xE872;</i></a>
        </td>";
            echo "</tr>";
  }
} else {
